...
# 🔑 Load Groq API key from environment variable (safer than hardcoding)
GROQ_API_KEY = os.getenv("GROQ_API_KEY")

# ✅ Reply formatting function
...
def chat():
    ...
    if not user_input:
        return jsonify({
            "reply": "Hi! How can I help with anime 🎥 or book 📚 suggestions today?"
        })
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Log the actual Groq error for debugging
            print("❌ Groq API error:", response.json())
            return jsonify({"reply": "MoodConexus Bot is temporarily unavailable. Please try again."}), 500

    except Exception as e:
        print("❌ Exception in /chat:", str(e))
        return jsonify({"reply": "Something went wrong while generating a response."}), 500
